...
class _AgendarLavagem extends State<AgendarLavagem> {
  ...
  var listaCarrosCliente = ['Selecione'];
        ...
        LavagemController().listarLavagemEspecifica(docId).then((value) {
          ...
          setState(() {
            ...
            horarioSelecionado = TimeOfDay(
                hour: int.parse(doc['horario'].toString().split(":")[0]),
                minute: int.parse(doc['horario'].toString().split(":")[1]));
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(docId == null ? 'Agendar lavagem' : 'Editar lavagem',
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              children: [
                if(listaCarrosCliente.isEmpty) const Center(
                            child: Text(
                ...
                if(listaCarrosCliente.isNotEmpty) Form(
                  key: formKey,
                  child: Column(
                    children: [
                      TextFormField(
                          ...
                          child: DropdownButton(
                            ...
                            onChanged: (String? novoValor) {
                              setState(() {
                                ...
                                if (listaCarrosCliente.contains('Selecione') &&
                                    valorPadraoDropDownCarro != 'Selecione') {
                              ...
                              ElevatedButton(
                                ...
                                onPressed: () {
                                  if (formKey.currentState!.validate()) {
                                    if (valorPadraoDropDownCarro ==
                                        'Selecione') {
                                      dialogBox(context, 'Erro',
                                          'Selecione o veículo');
                                    } else {
                                      Future<
                                              QuerySnapshot<
                                                  Map<String, dynamic>>>
                                          future = CarroController()
                                              .listarCarroNomeEspecifico(
                                                  valorPadraoDropDownCarro);
                                      future.then((value) {
                                        ...
                                        if (docId == null) {
                                          LavagemController()
                                              .agendarLavagem(context, l);
                                        } else {
                                          LavagemController()
                                              .editarLavagem(context, l, docId);
                                        }
                                      });
                                    }
                                  }
                            ...
                            onPressed: () {
                              if (formKey.currentState!.validate()) {
                                if (valorPadraoDropDownCarro == 'Selecione') {
                                  dialogBox(
                                      context, 'Erro', 'Selecione o veículo');
                                } else {
                                  LavagemController()
